...
async def _seed_bubbles_catalog(
    client: AsyncClient,
...
async def test_world_batch_max_100_frames(
    ...
    game_id, _preset_id, level_id = await _seed_bubbles_catalog(client, admin_headers)
    ...
    r = await client.post(
        f"/api/v1/bubbles/matches/{match_id}/telemetry/world",
        headers=player_headers,
        json={"frames": frames},
    )
...
async def test_pose_batch_max_100_frames(
    ...
    r = await client.post(
        f"/api/v1/bubbles/matches/{match_id}/telemetry/pose",
        headers=player_headers,
        json={"frames": frames},
    )
    assert r.status_code == 201
...
async def test_events_batch_max_500(
    ...
    r = await client.post(
        f"/api/v1/bubbles/matches/{match_id}/events",
        headers=player_headers,
        json={"events": events},
    )
    assert r.status_code == 201
...
async def test_world_wrong_game_match_returns_403(
    ...
    await _seed_bubbles_catalog(client, admin_headers)
    ...
    r = await client.post(
        f"/api/v1/bubbles/matches/{trunk_match}/telemetry/world",
        headers=player_headers,
        json={
            "frames": [
    ...
    assert r.json()["code"] == "NOT_BUBBLES_MATCH"
...
async def test_world_unknown_match_returns_404(
    ...
    r = await client.post(
        f"/api/v1/bubbles/matches/{missing}/telemetry/world",
        headers=player_headers,
        json={
            "frames": [
                {"timestamp": 1, "device": "d1", "data": {}},
