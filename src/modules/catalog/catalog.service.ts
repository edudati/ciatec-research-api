...
const ACTIVE_NOT_DELETED = { isActive: true, isDeleted: false } as const;
  ...
  return {
    ...
    async getGame(gameId: string) {
      const game = await prisma.game.findFirst({
        where: { id: gameId, ...ACTIVE_NOT_DELETED },
        select: { id: true, name: true, description: true, isActive: true, createdAt: true, updatedAt: true },
      });
    ...
    async updateGame(gameId: string, input: { name?: string; description?: string | null; isActive?: boolean }) {
      const existing = await prisma.game.findFirst({ where: { id: gameId, ...ACTIVE_NOT_DELETED }, select: { id: true } });
    ...
    async listPresets(gameId: string) {
      ...
      const presets = await prisma.preset.findMany({
        where: { gameId, ...ACTIVE_NOT_DELETED },
    ...
    async getPreset(presetId: string) {
      const preset = await prisma.preset.findFirst({
        where: { id: presetId, ...ACTIVE_NOT_DELETED, game: ACTIVE_NOT_DELETED },
        select: {
          id: true,
          gameId: true,
          name: true,
    ...
    async createPreset(input: {
      ...
      const game = await prisma.game.findFirst({ where: { id: input.gameId, ...ACTIVE_NOT_DELETED }, select: { id: true } });
      if (!game) throw new NotFoundError('Game not found');
    ...
    async deletePreset(presetId: string) {
      // Soft delete cascade: preset -> levels
      const preset = await prisma.preset.findFirst({
        where: { id: presetId, ...ACTIVE_NOT_DELETED, game: ACTIVE_NOT_DELETED },
        select: { id: true },
      });
    ...
    async listLevels(presetId: string) {
      ...
      const levels = await prisma.level.findMany({
        where: { presetId, ...ACTIVE_NOT_DELETED },
        orderBy: { order: 'asc' },
        select: {
          id: true,
          presetId: true,
    ...
    async getLevel(levelId: string) {
      const level = await prisma.level.findFirst({
        where: { id: levelId, ...ACTIVE_NOT_DELETED, preset: { ...ACTIVE_NOT_DELETED, game: ACTIVE_NOT_DELETED } },
        select: {
          id: true,
          presetId: true,
          name: true,
    ...
    async createLevel(input: {
      ...
      const preset = await prisma.preset.findFirst({
        where: { id: input.presetId, ...ACTIVE_NOT_DELETED, game: ACTIVE_NOT_DELETED },
        select: { id: true },
      });
      if (!preset) throw new NotFoundError('Preset not found');
